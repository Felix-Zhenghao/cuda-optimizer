...
A similar synchronization effect can also be achieved with CUDA events ([CUDA Events](../02-basics/asynchronous-execution.html#cuda-events)), e.g., by recording an event on that stream and calling
...
[Table 4](#table-streams-event-sync-summary) provides a high-level summary of various synchronization options
with the host.
...
For synchronization, i.e., to express dependencies, between CUDA streams, use of non-timing CUDA events is recommended, as described in [CUDA Events](../02-basics/asynchronous-execution.html#cuda-events).
...
![Programmatic Dependent Kernel Launch](img/Programmatic-Dependent-Kernel-Launch.png)
...
CUDA provides various environment variables (see [Section 5.2](../05-appendices/environment-variables.html#cuda-environment-variables)), which can affect execution and performance.
...
Please refer to [CUDA Environment Variables](../05-appendices/environment-variables.html#cuda-environment-variables) for more details about the various CUDA environment variables.
