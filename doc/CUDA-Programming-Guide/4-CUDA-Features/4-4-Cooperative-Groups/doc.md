...
| stride\_partition | Divides parent group into equally-sized subgroups where threads are assigned to subgroups in a round-robin manner. |
...
For details see the [Large Scale Groups](#cooperative-groups-large-scale-groups) section.
...
All threads in the group must pass the same values for corresponding arguments to each collective call, unless different values are explicitly allowed in the [Cooperative Groups API](../05-appendices/device-callable-apis.html#cg-api-partition-header).
