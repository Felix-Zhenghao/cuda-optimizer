...
A typical GPU activity timeline is shown in [Figure 39](#gpu-activity).

![GPU activity timeline](img/Figure-39-GPU-activity-timeline.png)
...
both of them can be launched concurrently by using [CUDA Streams](../02-basics/asynchronous-execution.html#cuda-streams).
...
![Preamble section of ``secondary_kernel``](img/Figure-40-Preamble-section-ofsecondary_kernel.png)

*Figure 40 Preamble section ofsecondary\_kernel*

[Figure 40](#secondary-kernel-preamble) demonstrates the portion of `secondary_kernel` that could
...
![Concurrent execution of ``primary_kernel`` and ``secondary_kernel``](img/Figure-41-Concurrent-execution-ofprimary_kernelandsecondary_kernel.png)

*Figure 41 Concurrent execution ofprimary\_kernelandsecondary\_kernel*

The concurrent launch and execution of `secondary_kernel` shown in [Figure 41](#preamble-overlap) is
...
Programmatic Dependent Launch can be used in [CUDA Graphs](cuda-graphs.html#cuda-graphs) via [stream capture](cuda-graphs.html#cuda-graphs-creating-a-graph-using-stream-capture) or
directly via [edge data](cuda-graphs.html#cuda-graphs-edge-data). To program this feature in a CUDA Graph with edge data, use a `cudaGraphDependencyType`
