...
For kernels launched as part of a CUDA graph (see [CUDA Graphs](cuda-graphs.html#cuda-graphs)), there are a few more subtleties.
...
Kernels with thread block clusters (see [Section 1.2.2.1.1](../01-introduction/programming-model.html#programming-model-thread-block-clusters)) can also be launched on a green context stream, like any other kernel, and thus use that green context’s provisioned resources.
[Section 4.6.4.2](#green-contexts-creation-example-step2) showed how to specify the number of SMs that need to be coscheduled when a device resource is split, to facilitate clusters.
...
[Figure 45](#green-contexts-ncu-mask) depicts these resources for an example with two green contexts provisioned with 112 and 16 SMs respectively, with no SM overlap across them.
...
![Green Contexts Resources in Nsight Compute](img/Figure-45-Green-contexts-resources-section-from-Nsight-Compute.png)
...
Similar to the scenario used in [Section 4.6.1](#green-contexts-motivation), the application has two kernels that will run on two different non-blocking CUDA streams.
...
[Figure 46](#green-contexts-nsys-example-no-gcs-with-prio) shows this scenario in an Nsight Systems report.
...
![Nsight Systems timeline without green contexts](img/Figure-46-Nsight-Systems-timeline-without-green-contexts.png)
...
As [Figure 47](#green-contexts-nsys-example-w-gcs) shows, the critical kernel can now start almost instantaneously, as there are SMs only Green Context 3 can use.
...
![Nsight Systems timeline with green contexts](img/Figure-47-Nsight-Systems-timeline-with-green-contexts.png)
