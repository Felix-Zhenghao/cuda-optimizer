...
* *Preemption*
  ...
  [higher-priority kernel](../02-basics/asynchronous-execution.html#async-execution-stream-priorities),
...
![Cluster Launch Control Flow](img/Figure-51-Cluster-Launch-Control-Flow.png)
...
[figure](#cluster-launch-control-diagram) above presents the execution
...
to [asynchronous data copies](../03-advanced/advanced-kernel-programming.html#advanced-kernels-async-copies).
...
3. Submit asynchronous cancellation request by a single thread and
   ...
   [invoke\_one](cooperative-groups.html#cooperative-groups-invoke-one) thread selector.
...
In the case of a [thread block clusters](../02-basics/intro-to-cuda-cpp.html#thread-block-clusters),
...
* Cancelling in the cluster case requires all the thread blocks to exist.
  ...
  `cg::cluster_group::sync()` from [sync](../05-appendices/device-callable-apis.html#cg-api-sync-function) API.
