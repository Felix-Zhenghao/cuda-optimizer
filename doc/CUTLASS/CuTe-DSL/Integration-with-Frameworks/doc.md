...
* `t5` shows automatic deduction for tensor `b` (multiple stride-1, exactly one has size > 1 â dim 1).
...
For more details, see [Compile with TVM FFI](compile_with_tvm_ffi.html).
