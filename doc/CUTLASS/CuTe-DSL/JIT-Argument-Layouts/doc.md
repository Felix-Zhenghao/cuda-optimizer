...
compiled_func(b_pack)  # â This results in an unexpected result at runtime due to type mismatch
```
...
compiled_func_2(b_pack)                      # â Now this works fine
```
...
Refer to [Integration with Frameworks](framework_integration.html) for more details on `from_dlpack`, `mark_layout_dynamic`,
...
Please refer to [Control Flow](dsl_control_flow.html) for more details.
