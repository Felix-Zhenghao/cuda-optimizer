...
A 3-D tensor is commonly used for matrix multiplications, with three letters: B, M, and N. B represents the batch size (for batch matmul), M represents the number of rows, and N represents the number of columns. Refer to the [CUDNN\_BACKEND\_OPERATION\_MATMUL\_DESCRIPTOR](../../../backend/v9.20.0/api/cudnn-graph-library.html#cudnn-backend-operation-matmul-descriptor "(in NVIDIA cuDNN Backend)") operation for more information.
...
The recommended way to specify the layout format of a tensor is by setting its strides accordingly. For compatibility with the v7 API, a subset of the layout formats can also be configured through the [cudnnTensorFormat\_t](../../../backend/v9.20.0/api/cudnn-graph-library.html#cudnntensorformat-t "(in NVIDIA cuDNN Backend)") enum. The enum is only supplied for legacy reasons and is deprecated.
...
![Example with N=1, C=64, H=5, W=4](img/Example-with-N1-C64-H5-W4.png)
...
![NCHW Memory Layout](img/NCHW-Memory-Layout.png)
...
![NHWC Memory Layout](img/NHWC-Memory-Layout.png)
...
![NC/32HW32 Memory Layout](img/NC32HW32-Memory-Layout.png)
...
> * The tensor format [cudnnTensorFormat\_t](../../../backend/v9.20.0/api/cudnn-graph-library.html#cudnntensorformat-t "(in NVIDIA cuDNN Backend)") can also be interpreted in the following way - The NCHW INT8x32 format is really `N x (C/32) x H x W x 32` (32 ``` C``s for every ``W ```), just as the NCHW INT8x4 format is `N x (C/4) x H x W x 4` (4 `C` for every `W`). Hence the `VECT_C` name - each `W` is a vector (4 or 32) of `C`.

#### Matmul Layouts

As discussed in [3-D Tensor Descriptor](#three-d-tensor-descriptor), matmul uses 3D tensors, described using BMN dimensions. The layout can be specified through the following strides. The following are two examples of recommended layouts:
