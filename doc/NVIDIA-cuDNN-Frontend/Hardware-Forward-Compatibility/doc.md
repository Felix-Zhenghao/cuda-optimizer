...
Every release, the cuDNN library has a list of SM versions that it supports natively, listed in the [cuDNN Support Matrix](../reference/support-matrix.html#support-matrix). When running on one of those SM versions, the library is in *native mode*. By native support, we mean the library includes explicit CUDA kernels specifically compiled for that SM architecture.
...
* [Pre-Compiled Single Operation Engines](graph-api.html#compile-single-op-engine)
...
* [Generic Runtime Fusion Engines](graph-api.html#runtime-fusion-engine)
* [Specialized Pre-Compiled Engines](graph-api.html#compile-specialized-engine)

  > + [ConvBiasAct](graph-api.html#convbiasact)
  > + [ConvScaleBiasAct](graph-api.html#convscalebiasact)
  > + [ConvBNFprop](graph-api.html#convbnfprop)
...
* [Specialized Runtime Fusion Engines](graph-api.html#specialized-runtime-fusion-engines) for graph patterns of Fused Attention and Fused Flash Attention (`fprop` and `bprop`)
* [Specialized Pre-Compiled Engines](graph-api.html#compile-specialized-engine) for graph patterns of:

  > + [ConvBNFprop](graph-api.html#convbnfprop): DBARCS pattern (short for Dual, Scale, Bias, Add, ReLU, Conv genStats)
  > + [ConvBNWgrad](graph-api.html#convbnwgrad)
  > + [DgradDreluBNBwdWeight](graph-api.html#dgraddrelubnbwdweight)
  > + [FP8 fused flash attention](graph-api.html#flash-fused-multi-head-att-fp8)
* Any graph operation involving the FP8 data type

For operation graphs that are not yet forward compatible, the library returns a [status](../../../backend/v9.20.0/api/cudnn-graph-library.html#cudnnstatus-t "(in NVIDIA cuDNN Backend)") of `CUDNN_STATUS_NOT_SUPPORTED_ARCH_MISMATCH`.
...
>
>   > + Convolution Forward operation or data gradient operation followed by operations of g 2 (in [Generic Runtime Fusion Engines](graph-api.html#runtime-fusion-engine)) supports only broadcasting to full tensors and does not support broadcasting between vector or scalar tensors. Using the latter may produce incorrect results.
>   > + Fusion patterns involving [grouped convolutions](../../../backend/v9.20.0/developer/misc.html#grouped-convolutions "(in NVIDIA cuDNN Backend)"), that is convolution with G>1, are not supported for data gradient and weight gradient operations.
>   > + Convolution Forward operation followed by operations of g 2 (in [Generic Runtime Fusion Engines](graph-api.html#runtime-fusion-engine)) involving `CUDNN_BACKEND_OPERATION_REDUCTION_DESCRIPTOR` does not support the following patterns of reduction:
>   >
>   >   > - [N, K, P, Q] -> [N, 1, 1, 1]
>   >   > - [N, K, P, Q] -> [N, K, 1, 1]
> * Fusion patterns in g 1 (in [Generic Runtime Fusion Engines](graph-api.html#runtime-fusion-engine)) combining `CUDNN_BACKEND_OPERATION_POINTWISE_DESCRIPTOR` which involve `CUDNN_POINTWISE_MUL (pointwise:Scale)`, `CUDNN_POINTWISE_ADD (pointwise:Bias)`, `CUDNN_POINTWISE_RELU_FWD (pointwise:Relu)` modes support only data type of half I/O and compute type of float32 for `convolutionForward` / `ConvolutionBackwardFilter`, while `ConvolutionBackwardData` is not supported for any data type.
