...
\(
O[q] = \sum\_{b \in \mathcal{B}\_q} \sum\_{k \in \text{block}\_b} \text{softmax}\left(\frac{Q[q] \cdot K[k]^T}{\sqrt{D}}\right) V[k]
\)
...
\(
O = \text{softmax}\left(\frac{Q \cdot K^T}{\sqrt{D}} \cdot \alpha\_q \cdot \alpha\_k + \text{mask}\right) \cdot V \cdot \alpha\_v \cdot \alpha\_o^{-1}
\)

where \(\alpha\_q\), \(\alpha\_k\), \(\alpha\_v\), \(\alpha\_o\) are optional scaling factors for quantized inputs.
...
\(
O[q] = \sum\_{k : q - L \leq k \leq q + R} \text{softmax}\left(\frac{Q[q] \cdot K[k]^T}{\sqrt{D}}\right) V[k]
\)
...
\(
\text{block\_score}[q, b] = \sum\_{k \in \text{block}\_b} \exp\left(\frac{Q[q] \cdot K[k]^T}{\sqrt{D}} - \text{LSE}[q]\right)
\)
...
* **cum\_seqlen**: `(batch_size + 1,)` – cumulative sequence lengths, e.g., `[0, 128, 320, 512]` for 3 sequences of lengths 128, 192, 192
