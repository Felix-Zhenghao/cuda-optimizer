...
**Legacy dense-only API note:** This page documents the older dense-only grouped quant API. For new integrations, prefer the unified [Grouped GEMM + Quant (Unified)](grouped_gemm_quant_unified.html) page.
...
\(
\text{ref}[m, n] = \alpha\_g \sum\_{k} \text{dequantize}(A[m, k], \text{SFA}) \cdot \text{dequantize}(B[n, k, g], \text{SFB})
\)
...
Let \(\text{rcp\_max} = 1 / q\_{\max}\), where \(q\_{\max}\) is the maximum representable value of the output data type (e.g., 448 for FP8 E4M3, 57344 for FP8 E5M2).
