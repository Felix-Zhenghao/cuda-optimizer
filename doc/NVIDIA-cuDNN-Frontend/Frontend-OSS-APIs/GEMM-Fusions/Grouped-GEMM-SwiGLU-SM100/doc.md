...
**Legacy contiguous-only API note:** This page documents the older contiguous-only SwiGLU API. For new integrations, prefer the unified [Grouped GEMM + GLU](grouped_gemm_glu.html) API, which covers dense and discrete weight layouts.
...
\(
C[m, n] = \alpha\_g \sum\_{k} \text{dequantize}(A[m, k], \text{SFA}) \cdot \text{dequantize}(B[n, k, g], \text{SFB})
\)
...
\(
D[:, bG:(b+1)G] = \text{prob} \cdot X\_b \cdot \text{swish}(G\_b), \quad \text{swish}(x) = x \cdot \sigma(x)
\)
