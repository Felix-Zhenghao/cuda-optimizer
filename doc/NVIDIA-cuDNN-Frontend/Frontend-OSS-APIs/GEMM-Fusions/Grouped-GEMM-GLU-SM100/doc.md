...
\(
D[:, bG:(b+1)G] = \text{prob} \cdot (X\_b + 1) \cdot G\_b \cdot \sigma(1.702 \cdot G\_b)
\)
...
* **Output tensor D\_col**: `d_col_tensor` / `sample_d_col`

  + Shape: `(valid_m, N/2, 1)` – must match D dtype and stride
* **Scale factor tensors**: Same as contiguous swiglu (SFA, SFB, SFD\_row, SFD\_col)
...
* `m_aligned`: Must be `256` (FIX\_PAD\_SIZE). Default: `256`
...
* Scale factor tensors (SFA, SFB, SFD\_row, SFD\_col) must have the same dtype
