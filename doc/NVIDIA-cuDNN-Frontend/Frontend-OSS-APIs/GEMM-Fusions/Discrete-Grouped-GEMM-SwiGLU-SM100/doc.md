...
\(
C[m, n] = \alpha\_g \sum\_{k} \text{dequantize}(A[m, k], \text{SFA}) \cdot \text{dequantize}(B\_g[n, k], \text{SFB}\_g)
\)
...
\(
D[:, bG:(b+1)G] = \text{prob} \cdot Up\_b \cdot \text{swish}(Gate\_b), \quad \text{swish}(x) = x \cdot \sigma(x)
\)
