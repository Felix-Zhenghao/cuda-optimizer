...
`tcgen05.dealloc` is a potentially blocking instruction which deallocates the [Tensor Memory](#tensor-memory)
...
* issuing warp and [peer CTA](#tcgen05-peer-cta) warp must synchronize [Tensor Memory](#tensor-memory) accesses
  before attempting to collectively deallocate the [Tensor Memory](#tensor-memory), and
* `tcgen05.dealloc` may block to collectively performs the deallocation with
  the other [peer CTA](#tcgen05-peer-cta)'s warp.
